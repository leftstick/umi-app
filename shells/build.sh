...
if  [ ! -n "$UMI_ENV" ] ;then
    output="umi-app-zip-dist"
else
    output="${UMI_ENV}-umi-app-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./umi-app*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/umi-app/'${UMI_ENV}'_umi-app/g' ./${output}/package.json
  else 
    sed -i 's/umi-app/'${UMI_ENV}'_umi-app/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/umi-app/'${UMI_ENV}'_umi-app/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/umi-app/'${UMI_ENV}'_umi-app/g' ./${output}/bin/stop.sh
  fi
fi
